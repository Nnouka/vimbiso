...
test_1.test.describe('Pattern Watch tab (DASH-3)', () => {
    test_1.test.beforeEach(async ({ page }) => {
        await login(page);
        await page.goto('/dashboard/pattern-watch');
    });
    (0, test_1.test)('a seeded match renders in the Pattern Watch tab', async ({ page }) => {
        const matchRows = page.getByTestId('pattern-match-row');
        await (0, test_1.expect)(matchRows.first()).toBeVisible();
    });
    (0, test_1.test)('the Pattern Watch tab carries no urgent/red styling (non-urgent institutional signal, per backlog.md DASH-3)', async ({ page }) => {
        ...
        const highFlaggedInPatternWatch = page.locator('[data-testid="pattern-match-row"][data-risk="HIGH"]');
        await (0, test_1.expect)(highFlaggedInPatternWatch).toHaveCount(0);
    });
    (0, test_1.test)('clicking "Mark reviewed" updates the match status', async ({ page }) => {
        const firstRow = page.getByTestId('pattern-match-row').first();
        const reviewButton = firstRow.getByTestId('mark-reviewed-btn');
        await (0, test_1.expect)(reviewButton).toBeVisible();
        ...
        await (0, test_1.expect)(firstRow).toContainText(/reviewed/i);
    });
});
//# sourceMappingURL=pattern-watch.spec.js.map