...
test_1.test.describe('Report detail view (DASH-4)', () => {
    test_1.test.beforeEach(async ({ page }) => {
        await login(page);
    });
    (0, test_1.test)('clicking a report row navigates to its detail view showing report-detail with triage data', async ({ page }) => {
        const firstRow = page.getByTestId('report-row').first();
        await (0, test_1.expect)(firstRow).toBeVisible();
        await firstRow.click();
        await (0, test_1.expect)(page).toHaveURL(/\/dashboard\/reports\/[^/]+$/);
        const detail = page.getByTestId('report-detail');
        await (0, test_1.expect)(detail).toBeVisible();
        ...
        await (0, test_1.expect)(detail).toContainText(/HIGH|STANDARD/);
        const questionKeys = [
            'STRANGLE',
            'WEAPON',
            'KILL_THREAT',
        ...
        (0, test_1.expect)(questionKeys.length).toBe(8);
    });
    (0, test_1.test)('report detail shows any sms_alerts timestamp when the underlying report triggered an SMS alert', async ({ page }) => {
        ...
        const detail = page.getByTestId('report-detail');
        await (0, test_1.expect)(detail).toBeVisible();
        await (0, test_1.expect)(detail).toContainText(/\d{4}-\d{2}-\d{2}/); // an ISO-ish date/timestamp is rendered somewhere
    });
});
//# sourceMappingURL=report-detail.spec.js.map