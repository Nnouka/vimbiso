...
const QUESTION_KEY_SET = new Set(QUESTION_ORDER);
function isTriageKey(key) {
...
// one new screen.
const KNOWN_INTERACTIVE_IDS = new Set([
    'lang_en',
    'lang_sw',
    'lang_fr',
    'menu_report',
...
// digit unrecognized.
const NUMBERED_OPTIONS_BY_STEP = {
 ...
 */
function numberedOptionsForStep(currentStep) {
    if (!currentStep)
        return null;
    if (currentStep.startsWith(TRIAGE_STEP_PREFIX))
        return TRIAGE_NUMBERED_OPTIONS;
...
async function getState(whatsappNumber) {
    const { rows } = await (0, db_1.query)('SELECT * FROM conversation_state WHERE whatsapp_number = $1', [whatsappNumber]);
    return rows[0] || null;
}
/**
...
async function saveState(whatsappNumber, current, patch) {
    const merged = {
        ...
        disclosure_shown: 'disclosure_shown' in patch ? patch.disclosure_shown ?? false : current?.disclosure_shown ?? false,
    };
    await (0, db_1.query)(`INSERT INTO conversation_state
...
async function sendLanguageSelector(to) {
    ...
    const sections = [
        {
            title: (0, content_1.t)('lang_select.section_title', lang),
            rows: [
                { id: 'lang_en', title: (0, content_1.t)('lang_select.row_english', lang) },
                { id: 'lang_sw', title: (0, content_1.t)('lang_select.row_kiswahili', lang) },
                { id: 'lang_fr', title: (0, content_1.t)('lang_select.row_francais', lang) },
            ],
        },
    ];
    await (0, whatsapp_1.sendList)(to, (0, content_1.t)('lang_select.body', lang), sections, (0, content_1.t)('lang_select.button', lang));
}
async function sendMainMenu(to, language) {
    ...
    const body = `${(0, content_1.t)('menu.body', language)}\n\n${(0, content_1.t)('menu.footer_hint', language)}`;
    const sections = [
        {
            title: (0, content_1.t)('menu.section_title', language),
            rows: [
                { id: 'menu_report', title: (0, content_1.t)('menu.report_row', language) },
                { id: 'menu_find_help', title: (0, content_1.t)('menu.find_help_row', language) },
                { id: 'menu_rights', title: (0, content_1.t)('menu.rights_row', language) },
                { id: 'menu_trusted_contact', title: (0, content_1.t)('menu.trusted_contact_row', language) },
            ],
        },
    ];
    await (0, whatsapp_1.sendList)(to, body, sections, (0, content_1.t)('menu.button', language));
}
async function sendQuestion(to, language, questionKey) {
    const prefix = `triage.${questionKey.toLowerCase()}`;
    const body = (0, content_1.t)(`${prefix}.question`, language);
    const buttons = [
        { id: 'triage_yes', title: (0, content_1.t)(`${prefix}.btn_yes`, language) },
        { id: 'triage_no', title: (0, content_1.t)(`${prefix}.btn_no`, language) },
        { id: 'triage_skip', title: (0, content_1.t)(`${prefix}.btn_skip`, language) },
    ];
    await (0, whatsapp_1.sendButtons)(to, body, buttons);
}
async function fetchNationalHotlineResource() {
    const { rows } = await (0, db_1.query)(`SELECT name, phone, source_name, last_verified_date
     FROM resources
...
async function fetchRegionResource(region) {
    const { rows } = await (0, db_1.query)(`SELECT name, phone, source_name, last_verified_date
     FROM resources
...
// ---------------------------------------------------------------------------
async function sendHighRiskSafetyPlan(to, language) {
    await (0, whatsapp_1.sendText)(to, (0, content_1.t)('highrisk.intro', language));
    // conversation-design.md §8.2: one message, all 4 items, fixed numbered
    // order — not 4 separate sends.
    const planLines = [1, 2, 3, 4].map((n) => `${n}. ${(0, content_1.t)(`highrisk.plan_${n}`, language)}`);
    await (0, whatsapp_1.sendText)(to, planLines.join('\n'));
    ...
    if (!hotline || !hotline.phone) {
        console.error('conversation.ts: HR-1 found no resources row for category=hotline, country=KE — ' +
        ...
        const prefixTemplate = (0, content_1.t)('highrisk.hotline_prefix', language);
        await (0, whatsapp_1.sendText)(to, prefixTemplate.replace('{{hotline_number}}', hotline.phone));
    }
}
async function sendConnectPrompt(to, language) {
    await (0, whatsapp_1.sendButtons)(to, (0, content_1.t)('highrisk.connect_prompt', language), [
        { id: 'connect_yes', title: (0, content_1.t)(BTN_YES, language) },
        { id: 'connect_no', title: (0, content_1.t)(BTN_NO, language) },
    ]);
}
/**
 ...
 */
async function maybeAlertTrustedContact(survivorWhatsappNumber) {
    let contact;
    try {
        const { rows } = await (0, db_1.query)('SELECT id, contact_whatsapp_number FROM trusted_contacts WHERE survivor_whatsapp_number = $1', [survivorWhatsappNumber]);
        contact = rows[0];
    }
    catch (err) {
    ...
    try {
        ...
        await (0, whatsapp_1.sendText)(contact.contact_whatsapp_number, (0, content_1.t)('trusted_contact.alert_message', 'en'));
        await (0, db_1.query)('UPDATE trusted_contacts SET alert_sent_at = now() WHERE id = $1', [contact.id]);
    }
    catch (err) {
        console.error(`conversation.ts: HR-4 failed to alert trusted contact for survivor ${survivorWhatsappNumber}:`, err);
    }
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
async function sendRegionPicker(to, language) {
    const sections = [
        {
            title: (0, content_1.t)('dir.region_section_title', language),
            rows: [
                { id: 'region_nairobi', title: (0, content_1.t)('dir.region_nairobi', language) },
                { id: 'region_mombasa', title: (0, content_1.t)('dir.region_mombasa', language) },
                { id: 'region_other', title: (0, content_1.t)('dir.region_other', language) },
            ],
        },
    ];
    await (0, whatsapp_1.sendList)(to, (0, content_1.t)('dir.region_prompt', language), sections, (0, content_1.t)('dir.region_button', language));
}
const REGION_NAME_BY_ROW_ID = {
 ...
 */
async function sendRegionResult(to, language, regionRowId) {
    const regionName = REGION_NAME_BY_ROW_ID[regionRowId];
    let resource = regionName ? await fetchRegionResource(regionName) : null;
    if (!resource) {
    ...
    if (!resource || !resource.name || !resource.phone) {
        ...
        // guarantee.
        console.error(`conversation.ts: DIR-2 found no usable resources row for region="${regionName || regionRowId}" ` +
            `or the national hotline fallback — has server/seeds/resources_kenya.ts been run?`);
        return;
    }
    const template = (0, content_1.t)('dir.result_format', language);
    const body = template
    ...
    await (0, whatsapp_1.sendText)(to, body);
}
// ---------------------------------------------------------------------------
// DIR-3: "Know your rights" example content
// ---------------------------------------------------------------------------
async function sendRightsContent(to, language) {
    ...
    const body = [
        (0, content_1.t)('rights.disclaimer', language),
        '',
        `1. ${(0, content_1.t)('rights.point_1', language)}`,
        `2. ${(0, content_1.t)('rights.point_2', language)}`,
        `3. ${(0, content_1.t)('rights.point_3', language)}`,
    ].join('\n');
    await (0, whatsapp_1.sendText)(to, body);
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
async function sendPwConsentPrompt(to, language) {
    await (0, whatsapp_1.sendButtons)(to, (0, content_1.t)('pw.consent_prompt', language), [
        { id: 'pw_consent_yes', title: (0, content_1.t)(BTN_YES, language) },
        { id: 'pw_consent_no', title: (0, content_1.t)(BTN_NO, language) },
    ]);
}
async function advanceToPwConsent(to, language, reportId) {
...
async function startTriage(from, language) {
    ...
    const { rows } = await (0, db_1.query)(`INSERT INTO reports (channel, language, status, whatsapp_number)
     VALUES ('whatsapp', $1, 'IN_PROGRESS', $2)
     RETURNING id`, [language, from]);
    const reportId = rows[0].id;
    const firstKey = QUESTION_ORDER[0];
    await (0, whatsapp_1.sendText)(from, (0, content_1.t)('triage.intro', language));
...
async function finishTriage(from, language, reportId) {
    const { rows } = await (0, db_1.query)('SELECT question_key, answer FROM triage_answers WHERE report_id = $1', [reportId]);
    const answers = {};
    ...
    const riskLevel = (0, riskScoring_1.scoreRisk)(answers);
    ...
    await (0, db_1.query)(`UPDATE reports SET risk_level = $1, status = 'SCORED' WHERE id = $2`, [
        riskLevel,
        reportId,
    ]);
    const bridgeKey = riskLevel === 'HIGH' ? 'highrisk.bridge' : 'standard.bridge';
    await (0, whatsapp_1.sendText)(from, (0, content_1.t)(bridgeKey, language));
 ...
 */
async function runHighRiskSequence(from, language, reportId) {
 ...
 */
async function runStandardRiskSequence(from, language, reportId) {
...
async function handleTriageAnswer(from, state, buttonId) {
    ...
    if (!currentStep || !isTriageKey(rawQuestionKey)) {
        console.error(`conversation.ts: handleTriageAnswer got an invalid/unrecognized current_step ` +
    ...
    if (!reportId) {
        ...
        // it to.
        console.error(`conversation.ts: TRIAGE state for ${from} has no report_id in temp_answers; resetting to MAIN_MENU.`);
        await sendMainMenu(from, language);
        return { current_step: 'MAIN_MENU', temp_answers: {} };
    }
    // TRI-1 AC: each answer written to triage_answers BEFORE advancing.
    await (0, db_1.query)('INSERT INTO triage_answers (report_id, question_key, answer) VALUES ($1, $2, $3)', [reportId, questionKey, answer]);
    ...
    if (nextKey === 'SEPARATION') {
        await (0, whatsapp_1.sendText)(from, (0, content_1.t)('triage.midpoint', language));
    }
...
// ---------------------------------------------------------------------------
async function handleConnectResponse(from, state, buttonId) {
    const language = state.language || 'en';
    const reportId = state.temp_answers?.report_id;
    if (!reportId) {
        // Defensive: mirrors handleTriageAnswer's "lost report_id" recovery below.
        console.error(`conversation.ts: AWAITING_CONNECT_RESPONSE state for ${from} has no report_id in ` +
    ...
    if (buttonId === 'connect_yes') {
        ...
        await (0, db_1.query)(`UPDATE reports SET wants_counsellor_connect = true, connect_requested_at = now() WHERE id = $1`, [reportId]);
        try {
            const results = await (0, sms_1.alertOnCallCounsellors)(reportId, 'HIGH');
            for (const result of results) {
                await (0, db_1.query)(`INSERT INTO sms_alerts (report_id, sent_to, sent_at, twilio_sid, status)
           VALUES ($1, $2, now(), $3, $4)`, [reportId, result.phone, result.sid, result.status]);
            }
        }
        catch (err) {
            ...
            await (0, db_1.query)(`INSERT INTO sms_alerts (report_id, sent_to, sent_at, twilio_sid, status)
         VALUES ($1, NULL, now(), NULL, 'failed')`, [reportId]);
        }
        await (0, whatsapp_1.sendText)(from, (0, content_1.t)('highrisk.connect_yes_ack', language));
    }
    else {
        ...
        await (0, whatsapp_1.sendText)(from, (0, content_1.t)('highrisk.connect_no_ack', language));
    }
...
async function handleRegionSelection(from, state, buttonId) {
    const language = state.language || 'en';
    ...
    if (reportId) {
        ...
        await (0, db_1.query)('UPDATE reports SET region = $1 WHERE id = $2', [regionName, reportId]);
    }
...
// ---------------------------------------------------------------------------
async function handlePwConsentResponse(from, state, buttonId) {
    const language = state.language || 'en';
    const reportId = state.temp_answers?.report_id;
    if (!reportId) {
        console.error(`conversation.ts: AWAITING_PW_CONSENT state for ${from} has no report_id in temp_answers; ` +
    ...
    if (buttonId === 'pw_consent_no') {
        ...
        await (0, whatsapp_1.sendText)(from, (0, content_1.t)('pw.consent_no_ack', language));
        return { current_step: 'MAIN_MENU', temp_answers: {} };
    }
    ...
    await (0, whatsapp_1.sendText)(from, (0, content_1.t)('pw.consent_yes_ack', language));
    return { current_step: 'AWAITING_PW_IDENTIFIER', temp_answers: { report_id: reportId } };
}
async function handlePwIdentifier(from, state, body) {
    const language = state.language || 'en';
    const reportId = state.temp_answers?.report_id;
    if (!reportId) {
        console.error(`conversation.ts: AWAITING_PW_IDENTIFIER state for ${from} has no report_id in temp_answers; ` +
            `resetting to MAIN_MENU.`);
        return { current_step: 'MAIN_MENU', temp_answers: {} };
    }
    ...
    await (0, db_1.query)(`UPDATE reports SET perpetrator_consent_given = true WHERE id = $1`, [reportId]);
    try {
        ...
        const hashValue = (0, hashing_1.normalizeAndHash)(body);
        await (0, db_1.query)(`INSERT INTO perpetrator_hashes (report_id, hash_value, algorithm) VALUES ($1, $2, 'HMAC-SHA256')`, [reportId, hashValue]);
        ...
        await (0, patternMatch_1.recordAndCheckPattern)(reportId, hashValue);
    }
    catch (err) {
 ...
 */
function isPlausibleWhatsappNumber(text) {
    return /^\+[0-9]{7,15}$/.test(text.trim());
}
async function handleTrustedContactNumber(from, state, body) {
    const language = state.language || 'en';
    if (typeof body !== 'string' || !isPlausibleWhatsappNumber(body)) {
        await (0, whatsapp_1.sendText)(from, (0, content_1.t)('trusted_contact.invalid_number', language));
    ...
    const { rows } = await (0, db_1.query)('SELECT id FROM trusted_contacts WHERE survivor_whatsapp_number = $1', [from]);
    if (rows.length > 0) {
        ...
        await (0, db_1.query)(`UPDATE trusted_contacts
       SET contact_whatsapp_number = $1, registered_at = now(), alert_sent_at = NULL
       WHERE id = $2`, [contactNumber, rows[0].id]);
    }
    else {
        await (0, db_1.query)(`INSERT INTO trusted_contacts (survivor_whatsapp_number, contact_whatsapp_number) VALUES ($1, $2)`, [from, contactNumber]);
    }
    await (0, whatsapp_1.sendText)(from, (0, content_1.t)('trusted_contact.confirm', language));
    return { current_step: 'MAIN_MENU', temp_answers: {} };
}
// ---------------------------------------------------------------------------
...
async function handleMainMenu(from, state, buttonId) {
    ...
    if (buttonId === 'menu_trusted_contact') {
        ...
        await (0, whatsapp_1.sendText)(from, (0, content_1.t)('trusted_contact.prompt', language));
        await (0, whatsapp_1.sendText)(from, (0, content_1.t)('trusted_contact.ask_number', language));
...
async function handleIncomingMessage(msg) {
    ...
    if (state && !stale && isDeviceSafetyTrigger(body)) {
        const language = state.language || 'en';
        await (0, whatsapp_1.sendText)(from, (0, content_1.t)('guidance.device_safety', language));
        await sendMainMenu(from, language);
    ...
    if (state.current_step === 'AWAITING_LANGUAGE') {
        ...
        if (!state.disclosure_shown) {
            await (0, whatsapp_1.sendText)(from, (0, content_1.t)('disclosure_message', language));
        }
        await sendMainMenu(from, language);
        ...
        console.warn(`conversation.ts: unrecognized current_step "${state.current_step}" for ${from}; resetting to MAIN_MENU.`);
