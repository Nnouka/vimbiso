...
exports.seedPatternWatchDemo = seedPatternWatchDemo;
...
const FAKE_REPORTS = [
    { whatsapp_number: 'whatsapp:+10000000001', language: 'en', risk_level: 'STANDARD' },
    { whatsapp_number: 'whatsapp:+10000000002', language: 'en', risk_level: 'HIGH' },
    { whatsapp_number: 'whatsapp:+10000000003', language: 'sw', risk_level: 'STANDARD' },
];
async function seedPatternWatchDemo() {
    const pool = (0, db_1.getPool)();
    const hashValue = (0, hashing_1.normalizeAndHash)(FAKE_IDENTIFIER_TEXT);
    const reportIds = [];
    for (const fake of FAKE_REPORTS) {
        ...
        const { rows: existing } = await pool.query(`SELECT id FROM reports WHERE whatsapp_number = $1 AND channel = 'demo-seed'`, [fake.whatsapp_number]);
        let reportId;
        if (existing.length > 0) {
            reportId = existing[0].id;
            await (0, db_1.query)(`UPDATE reports SET risk_level = $1, status = 'SCORED', perpetrator_consent_given = true WHERE id = $2`, [fake.risk_level, reportId]);
        }
        else {
            const { rows } = await (0, db_1.query)(`INSERT INTO reports (channel, language, risk_level, status, whatsapp_number, perpetrator_consent_given)
         VALUES ('demo-seed', $1, $2, 'SCORED', $3, true)
         RETURNING id`, [fake.language, fake.risk_level, fake.whatsapp_number]);
            reportId = rows[0].id;
        }
        ...
        const { rows: existingHash } = await pool.query('SELECT id FROM perpetrator_hashes WHERE report_id = $1', [reportId]);
    ...
    for (const reportId of reportIds) {
        lastDecision = await (0, patternMatch_1.recordAndCheckPattern)(reportId, hashValue);
    }
    console.log(`[seed] pattern_watch_demo: ${FAKE_REPORTS.length} obviously-fake demo reports ` +
...
//# sourceMappingURL=pattern_watch_demo.js.map