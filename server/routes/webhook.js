...
// all of that lives in server/lib/conversation.js (Backend-owned). This file
// also never reads Twilio credentials directly; all Twilio access goes
// through server/lib/whatsapp.js.
//
...
// server/lib/conversation.js does not exist yet as of this file landing —
// Backend creates it next, per the sprint sequencing in
// docs/sprint-1-plan.md §5. This `require` is intentional, not a mistake:
// the sprint plan calls for importing and wiring this call now ("import it
// anyway ... so the wiring is correct and ready the moment Backend's file
// lands"). Until server/lib/conversation.js exists, `require()`-ing this
// router module will throw MODULE_NOT_FOUND at load time. That is expected
// and temporary — see the summary this file shipped with for what that
// means for anyone trying to boot the server before Backend's file lands.
...
// global body-parser middleware (if any) server/index.js applies.
...
router.post('/webhook/whatsapp', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).send('Server misconfigured: ' + err.message);
  }
  ...
  //                   the normalized shape below — Backend's content.js
  ...
  const normalizedMsg = {
    ...
    buttonId: req.body.ButtonPayload || null,
