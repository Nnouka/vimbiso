 ...
 */
export async function recordAndCheckPattern(
  reportId: number,
  hashValue: string
): Promise<MatchDecision> {
  const { rows } = await query<{ report_id: number }>(
    'SELECT DISTINCT report_id FROM perpetrator_hashes WHERE hash_value = $1 AND report_id != $2',
    [hashValue, reportId]
  );
  ...
  if (decision.isMatch) {
    const { rows: existingRows } = await query<{ id: number; report_ids: number[] }>(
      'SELECT id, report_ids FROM pattern_matches WHERE hash_value = $1',
      [hashValue]
    );
    ...
    if (!existing) {
      await query(
        `INSERT INTO pattern_matches (hash_value, report_ids, status)
         VALUES ($1, $2, 'NEW')`,
        [hashValue, decision.allReportIds]
      );
    } else {
      ...
      await query('UPDATE pattern_matches SET report_ids = $1 WHERE id = $2', [merged, existing.id]);
    }
  }
