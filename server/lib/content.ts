...
/** Shape of one row in the `content_strings` table (see server/migrations/009_create_content_strings.sql). */
export interface ContentString {
  key: string;
  language: string;
  text: string;
  tier: 'FULL' | 'PARTIAL' | 'ARCHITECTURE_ONLY';
 ...
 */
export async function loadContentCache(): Promise<Map<string, string>> {
  const pool = getPool();
  const { rows } = await pool.query<Pick<ContentString, 'key' | 'language' | 'text'>>(
    'SELECT key, language, text FROM content_strings'
  );
...
function safeFallback(key: string): string {
...
export function t(key: string, language: string = 'en'): string {
  if (!cache) {
    console.warn(
      `content.ts: t("${key}", "${language}") called before loadContentCache() completed — ` +
  ...
  if (language !== 'en') {
    console.warn(
      `content.ts: missing content for key="${key}" language="${language}" — falling back to English.`
    );
  }
  ...
  console.warn(
    `content.ts: missing content for key="${key}" in language="${language}" AND in the English ` +
 ...
 */
export function assertCriticalContentSeeded(): void {
  if (!cache) return; // loadContentCache() itself already failed/warned loudly.

  const missing = CRITICAL_CONTENT_KEYS.filter((key) => !cache!.has(cacheKey(key, 'en')));
  if (missing.length === 0) return;

  console.error(
    `[startup] CRITICAL: content_strings is missing ${missing.length} of the ` +
