...
Built for a hackathon. Sprint 3 added genuine local-Postgres verification (see `docs/qa-sprint3-report.md`); as of 2026-09-19, the bot backend and the counsellor dashboard are both deployed to Render, against a real Twilio account, and a full live round-trip has been run and confirmed: a real WhatsApp message through the Twilio Sandbox drove the complete flow (language → triage → HIGH-risk score → safety plan), a real SMS reached the on-call counsellor's actual phone, and the resulting report appeared correctly — High-Risk pinned, masked phone, triage transcript — on the live, deployed dashboard. That closes what used to be this section's three biggest gaps (no live Twilio round-trip, no real SMS delivered, dashboard rendering unverified). What's still simulated, honestly:
